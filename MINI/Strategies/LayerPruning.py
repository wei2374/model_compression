import tensorflow
from MINI.Core.Strategy import Strategy
from MINI.Core.Layer import Layer
from MINI.Layers.Conv2DLayer import Conv2DLayer
from MINI.Engines.TFEngine import TFEngine
from MINI.Evaluators.MagEval import MagEval
from MINI.Evaluators.SecGEval import SecGEval
from MINI.Evaluators.TaylorEval import TaylorEval
from MINI.Evaluators.ActEval import ActEval
import numpy as np
...
class LayerPruning(Strategy):
    def run(self, model, config):
        ...
        evaluator = TaylorEval(engine=TFEngine(), task=self.task)
        scores = evaluator.evaluate(model)
        filters = self.param_est(scores)
    ...
    def param_est(self, scores):
        #TODO::prune_ratio
        prune_ratio = 0.5
        filters = {}
        for sc in scores:
            score_sorted = np.sort(scores[sc])
            index = int(prune_ratio*len(score_sorted))
            prun_threshold = score_sorted[index]
            prune_mask = scores[sc] <= prun_threshold
                        ...
                        for next_layer in next_layers:
                            ...
                            if len(_next_layers)==1 and\
                               any([_next_layers[0].is_type(_layer_type) for _layer_type in layer.STOP_LAYERS]):
                                _next_layers[0].passive_prune(filter, model_params, layer_index_dic)
