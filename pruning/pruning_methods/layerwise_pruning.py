from pruning.delete_filters import delete_filter_after
from pruning.helper_functions import load_model_param
from pruning.pruning_criterions.magnitude import get_and_plot_weights
from pruning.pruning_criterions.activations import get_and_plot_activations
from pruning.pruning_criterions.apoz import get_and_plot_ApoZ
from pruning.pruning_criterions.second_gradient import get_and_plot_gradients2
from pruning.pruning_criterions.first_taylor import get_and_plot_taylor1
from pruning.pruning_criterions.batch_norm import get_and_plot_BN
from pruning.helper_functions import load_model_param
...
def channel_prune_model_layerwise(
    ...
    if criterion == "magnitude":
        ...
        score = get_and_plot_activations(
            my_model, layer_index_dic, dataset=dataset)
    ...
    with Bar(f'Layerwise channel pruning...') as bar:
        for index in target_layers:
            layer_params, _, prun_filter = prun_filters_layer(
                                index, layer_params, prune_ratio[index],
